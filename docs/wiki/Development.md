...
- **Scheduler tick (every 10s):** collect due users → fetch each *distinct* coin **once** → process users concurrently (`Semaphore(5)` + `gather`) → compare against `CryptoPreferences.last_price` → send only on ≥ 0.01% moves.
